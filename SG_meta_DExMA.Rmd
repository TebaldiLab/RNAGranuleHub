...
```{r DExMA-ObjectMA}
###There is a how-to from Villatoro-García: Differential Expression Meta-Analysis with DExMA package
...
enriched_filt <- resultsMA.anno %>%
  filter(AveFC >= 0.5 & Pval <= 0.05) %>%
  mutate(X = NULL, m6aNum.y = NULL, Biotype.y = NULL, Region.y = NULL) %>%
  rename(m6aNum = m6aNum.x, Biotype = Biotype.x, Region = Region.x)

write.csv(enriched_filt, "Meta-Analysis_enriched_genes.csv", row.names = TRUE)
...
enriched_genesMA <- enriched_filt %>%
...
pvals.FCE_df <- as.data.frame(pvals$logFC)

zhou.p <- pvals.p_df %>%
  dplyr::select(Zhou_UV_G3BP1)

zhou.FC <- pvals.FCE_df %>%
  dplyr::select(Zhou_UV_G3BP1)

zhou.p <- rownames_to_column(zhou.p, var = "gene")
zhou.FC <- rownames_to_column(zhou.FC, var = "gene")

zhou.results <- left_join(zhou.p, zhou.FC, by = "gene") %>%
  na.omit() %>%
  rename(p_val = Zhou_UV_G3BP1.x) %>%
  rename(FC = Zhou_UV_G3BP1.y )

EnhancedVolcano(zhou.results,
    lab = "gene",
    x = "FC",
    y = "p_val")
